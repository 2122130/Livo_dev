...
import { ROUTES, TAIOU_STATUS, TAIOU_CATEGORY, TAIOU_STATUS_LABELS, TAIOU_CATEGORY_LABELS } from '../../constants/index';
...
function TaiouTourokuForm() {
  ...
  // フォームの状態（定数の as const によるリテラル型固定を避けるため number で明示）
  const [formData, setFormData] = useState({
    bukken_id: '',
    room_id: '',
    uketsuke_date: new Date().toISOString().split('T')[0], // 初期値は今日の日付
    category: TAIOU_CATEGORY.SHUZEN as number,
    status: TAIOU_STATUS.MITAIOU as number,
    title: '',
    detail: '',
    completion_date: '',
    uketsukesha: '',
  ...
  const handleStatusChange = (statusValue: string) => {
    ...
    setFormData(prev => ({
      ...prev,
      status,
      completion_date: status === TAIOU_STATUS.KANRYOU && !prev.completion_date
        ? new Date().toISOString().split('T')[0]
        : status !== TAIOU_STATUS.KANRYOU ? '' : prev.completion_date
    }));
  };
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              >
                {Object.values(TAIOU_CATEGORY).map(cat => (
                  <option key={cat} value={cat}>{TAIOU_CATEGORY_LABELS[cat] ?? cat}</option>
                ))}
              </select>
            </div>
            ...
            <div>
              ...
              >
                {Object.values(TAIOU_STATUS).map(st => (
                  <option key={st} value={st}>{TAIOU_STATUS_LABELS[st] ?? st}</option>
                ))}
              </select>
            </div>
            ...
            <div>
              ...
              <input
                type="date"
                disabled={formData.status !== TAIOU_STATUS.KANRYOU}
